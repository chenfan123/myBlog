...
#### JSONL wire format (normative for streaming)

When server-to-client messages are carried as **JSON Lines (JSONL)**—one JSON object per line (e.g. over SSE)—the following applies:

- Each **line** MUST contain **one** complete server-to-client message object (`beginRendering`, `surfaceUpdate`, `dataModelUpdate`, or `deleteSurface`—each line exactly one of these).
- For every **`surfaceUpdate`** line, the `components` array MUST contain **exactly one** component object. Additional components for the same surface MUST be sent as **additional lines**, each line being its own `surfaceUpdate` with `surfaceId` set and `components` of length one.

This keeps each transport unit small so the client can parse and merge components as each line completes, without waiting for a large batched `surfaceUpdate`.

**Compatibility:** Clients MAY accept a legacy `surfaceUpdate` where `components` has more than one entry on a single line (or split such an array into logical single-component updates internally). Agents and generators that target streaming **SHOULD NOT** emit multiple components in one JSONL line.
...
### 5.5. Renderer-local `updateModel` simulation (non-standard extension)

A2UI v0.8 does not define a standard client-side mutation message for updating the local data model without a server round trip. In practice, some renderers may need optimistic/local interactions (for example, click a button and immediately update a bound text value).

For this scenario, a renderer MAY support a **non-standard local convention** in `action.context` and simulate a `dataModelUpdate` by calling SDK `updateModel` **in the action trigger** before dispatching `userAction` to the server. This is documented on `client_to_server.json` (`$defs.rendererLocalUpdateContext`); the on-wire `userAction` schema does not gain a new message type.

Reserved context keys used by this convention (component `action.context` in `surfaceUpdate`, not the emitted payload):

- `__localUpdatePath` (string): target bound path (for example `/demo/message`).
- `__localUpdateString` (string, optional): write string value.
- `__localUpdateNumber` (number, optional): write number value.
- `__localUpdateBoolean` (boolean, optional): write boolean value.
- `__openLinkUrl` (string, optional): open external URL locally in renderer.
- `__openLinkTarget` (string, optional): target window name (default `_blank`).

Behavior (`a2ui-react` action trigger):

1. Resolve `action.context` as normal.
2. If `__localUpdatePath` and one typed update value exist, call `updateModel(surfaceId, path, value)` so bound components re-render from `surface.dataModel`.
3. If `__openLinkUrl` is present, or `action.name` is `openLink` / `open_link`, open a safe `http:` / `https:` URL with `window.open(url, target, "noopener,noreferrer")`. Default target is `_blank`. Non-http(s) URLs (including `javascript:`) MUST be ignored.
4. Continue emitting `userAction` to server as usual, with reserved keys stripped from `context`.

Notes:

- This is renderer-specific behavior and not part of the normative v0.8 wire schema.
- Agents should treat these keys as optional implementation details and not rely on them unless a renderer contract explicitly documents support.
- A renderer may also map action names like `openLink` / `open_link` to local `window.open(...)` behavior.
