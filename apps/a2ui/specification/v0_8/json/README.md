...
-   `client_to_server.json`: This schema defines the structure for event messages sent from the client to the server. The on-wire payload is exactly one of `userAction` or `error`. v0.8 has no client→server mutation or navigation message; renderer-local extensions are documented in `$defs.rendererLocalUpdateContext` and `$defs.rendererLocalOpenLinkContext`. `a2ui-react` applies reserved `__localUpdate*` keys with SDK `updateModel`, and `openLink` / `__openLinkUrl` with `window.open`, then emits a normal `userAction` with those keys stripped.
...
-   `catalogs/minimal/minimal_catalog.json`: This is a strict subset of the standard catalog, containing only a core set of fundamental components (`Text`, `Row`, `Column`, `Button`, `TextField`). It is designed for testing new renderer implementations and serves as a simpler entry point for development.
