export function readSurfaceId(messages: readonly unknown[]): string {
...
/** 检查 A2UI 消息里被引用但尚未定义的组件 id。 */
export function missingComponentIds(messages: readonly unknown[]): string[] {
  ...
  for (const message of messages) {
    ...
    for (const item of update.components) {
      ...
      if (isRecord(item.component)) {
        for (const id of childIds(item.component)) {
          referenced.add(id);
        }
      }
    }
